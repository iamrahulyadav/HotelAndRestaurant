package com.shoaibnwar.hotelandrestaurant.Activities;

import android.app.Activity;
import android.content.Intent;
import android.support.v7.app.AppCompatActivity;
import android.os.Bundle;
import android.util.Log;
import android.view.View;
import android.widget.RelativeLayout;
import android.widget.TextView;

import com.shoaibnwar.hotelandrestaurant.R;

import java.util.ArrayList;
import java.util.HashMap;

public class AddMoreBreakfast extends AppCompatActivity {

    RelativeLayout rl_item_1, rl_item_2, rl_item_3, rl_item_4, rl_item_5, rl_item_6, rl_item_7, rl_item_8;

    TextView tv_item_name_1, tv_item_name_2, tv_item_name_3, tv_item_name_4, tv_item_name_5, tv_item_name_6, tv_item_name_7, tv_item_name_8;
    TextView tv_item_price_1, tv_item_price_2, tv_item_price_3, tv_item_price_4, tv_item_price_5, tv_item_price_6, tv_item_price_7, tv_item_price_8;
    RelativeLayout rl_back_arrow;
    ArrayList<HashMap<String, String>> dataList;
    
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_add_more_breakfast);
        
        init();
        clicksFunctions();
        backArrowPress();
    }
    
    private void init()
    {

        dataList = new ArrayList<>();
        rl_back_arrow = (RelativeLayout) findViewById(R.id.rl_back_arrow);

        rl_item_1 = (RelativeLayout) findViewById(R.id.rl_item_1);
        rl_item_2 = (RelativeLayout) findViewById(R.id.rl_item_2);
        rl_item_3 = (RelativeLayout) findViewById(R.id.rl_item_3);
        rl_item_4 = (RelativeLayout) findViewById(R.id.rl_item_4);
        rl_item_5 = (RelativeLayout) findViewById(R.id.rl_item_5);
        rl_item_6 = (RelativeLayout) findViewById(R.id.rl_item_6);
        rl_item_7 = (RelativeLayout) findViewById(R.id.rl_item_7);
        rl_item_8 = (RelativeLayout) findViewById(R.id.rl_item_8);

        tv_item_name_1 = (TextView) findViewById(R.id.tv_item_name_1);
        tv_item_name_2 = (TextView) findViewById(R.id.tv_item_name_2);
        tv_item_name_3 = (TextView) findViewById(R.id.tv_item_name_3);
        tv_item_name_4 = (TextView) findViewById(R.id.tv_item_name_4);
        tv_item_name_5 = (TextView) findViewById(R.id.tv_item_name_5);
        tv_item_name_6 = (TextView) findViewById(R.id.tv_item_name_6);
        tv_item_name_7 = (TextView) findViewById(R.id.tv_item_name_7);
        tv_item_name_8 = (TextView) findViewById(R.id.tv_item_name_8);

        tv_item_price_1 = (TextView) findViewById(R.id.tv_item_price_1);
        tv_item_price_2 = (TextView) findViewById(R.id.tv_item_price_2);
        tv_item_price_3 = (TextView) findViewById(R.id.tv_item_price_3);
        tv_item_price_4 = (TextView) findViewById(R.id.tv_item_price_4);
        tv_item_price_5 = (TextView) findViewById(R.id.tv_item_price_5);
        tv_item_price_6 = (TextView) findViewById(R.id.tv_item_price_6);
        tv_item_price_7 = (TextView) findViewById(R.id.tv_item_price_7);
        tv_item_price_8 = (TextView) findViewById(R.id.tv_item_price_8);


        dataList = (ArrayList<HashMap<String, String>>) getIntent().getSerializableExtra("mylist");
       for (int s=0;s<dataList.size();s++){
           String itemPostion = dataList.get(s).get("itemImagePosition");
           if (itemPostion.equals("1")){rl_item_1.setVisibility(View.GONE);}
           if (itemPostion.equals("2")){rl_item_2.setVisibility(View.GONE);}
           if (itemPostion.equals("3")){rl_item_3.setVisibility(View.GONE);}
           if (itemPostion.equals("4")){rl_item_4.setVisibility(View.GONE);}
           if (itemPostion.equals("5")){rl_item_5.setVisibility(View.GONE);}
           if (itemPostion.equals("6")){rl_item_6.setVisibility(View.GONE);}
           if (itemPostion.equals("7")){rl_item_7.setVisibility(View.GONE);}
           if (itemPostion.equals("8")){rl_item_8.setVisibility(View.GONE);}
       }
    }

    private void clicksFunctions(){
        item1Click();
        item2Click();
        item3Click();
        item4Click();
        item5Click();
        item6Click();
        item7Click();
        item8Click();
    }

    private void item1Click(){
        rl_item_1.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                String itemName = tv_item_name_1.getText().toString();
                String itemPrice = tv_item_price_1.getText().toString();

                Intent i = new Intent(AddMoreBreakfast.this, FoodSingleItem.class);
                i.putExtra("ImagePosition", "1");
                i.putExtra("itemName", itemName);
                i.putExtra("itemPrice", itemPrice);
                i.putExtra("empty", "no");
                setResult(Activity.RESULT_OK,i);
                finish();


            }
        });

    }
    private void item2Click(){
        rl_item_2.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                String itemName = tv_item_name_2.getText().toString();
                String itemPrice = tv_item_price_2.getText().toString();

                Intent i = new Intent(AddMoreBreakfast.this, FoodSingleItem.class);
                i.putExtra("ImagePosition", "2");
                i.putExtra("itemName", itemName);
                i.putExtra("itemPrice", itemPrice);
                i.putExtra("empty", "no");
                setResult(Activity.RESULT_OK,i);
                finish();

            }
        });
    }
    private void item3Click(){
        rl_item_3.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                String itemName = tv_item_name_3.getText().toString();
                String itemPrice = tv_item_price_3.getText().toString();

                Intent i = new Intent(AddMoreBreakfast.this, FoodSingleItem.class);
                i.putExtra("ImagePosition", "3");
                i.putExtra("itemName", itemName);
                i.putExtra("itemPrice", itemPrice);
                i.putExtra("empty", "no");
                setResult(Activity.RESULT_OK,i);
                finish();

            }
        });
    }
    private void item4Click(){
        rl_item_4.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String itemName = tv_item_name_4.getText().toString();
                String itemPrice = tv_item_price_4.getText().toString();

                Intent i = new Intent(AddMoreBreakfast.this, FoodSingleItem.class);
                i.putExtra("ImagePosition", "4");
                i.putExtra("itemName", itemName);
                i.putExtra("itemPrice", itemPrice);
                i.putExtra("empty", "no");
                setResult(Activity.RESULT_OK,i);
                finish();

            }
        });
    }
    private void item5Click(){
        rl_item_5.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                String itemName = tv_item_name_5.getText().toString();
                String itemPrice = tv_item_price_5.getText().toString();

                Intent i = new Intent(AddMoreBreakfast.this, FoodSingleItem.class);
                i.putExtra("ImagePosition", "5");
                i.putExtra("itemName", itemName);
                i.putExtra("itemPrice", itemPrice);
                i.putExtra("empty", "no");
                setResult(Activity.RESULT_OK,i);
                finish();

            }
        });
    }

    private void item6Click(){
        rl_item_6.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                String itemName = tv_item_name_6.getText().toString();
                String itemPrice = tv_item_price_6.getText().toString();

                Intent i = new Intent(AddMoreBreakfast.this, FoodSingleItem.class);
                i.putExtra("ImagePosition", "6");
                i.putExtra("itemName", itemName);
                i.putExtra("itemPrice", itemPrice);
                i.putExtra("empty", "no");
                setResult(Activity.RESULT_OK,i);
                finish();

            }
        });
    }
    private void item7Click(){
        rl_item_7.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                String itemName = tv_item_name_7.getText().toString();
                String itemPrice = tv_item_price_7.getText().toString();

                Intent i = new Intent(AddMoreBreakfast.this, FoodSingleItem.class);
                i.putExtra("ImagePosition", "7");
                i.putExtra("itemName", itemName);
                i.putExtra("itemPrice", itemPrice);
                i.putExtra("empty", "no");
                setResult(Activity.RESULT_OK,i);
                finish();

            }
        });
    }
    private void item8Click(){
        rl_item_8.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                String itemName = tv_item_name_8.getText().toString();
                String itemPrice = tv_item_price_8.getText().toString();

                Intent i = new Intent(AddMoreBreakfast.this, FoodSingleItem.class);
                i.putExtra("ImagePosition", "8");
                i.putExtra("itemName", itemName);
                i.putExtra("itemPrice", itemPrice);
                i.putExtra("empty", "no");
                setResult(Activity.RESULT_OK,i);
                finish();

            }
        });
    }

    private void backArrowPress(){
        rl_back_arrow.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent i = new Intent(AddMoreBreakfast.this, FoodSingleItem.class);
                i.putExtra("empty", "yes");
                setResult(Activity.RESULT_OK,i);
                finish();
            }
        });
    }

    @Override
    public void onBackPressed() {

        Intent i = new Intent(AddMoreBreakfast.this, FoodSingleItem.class);
        i.putExtra("empty", "yes");
        setResult(Activity.RESULT_OK,i);
        super.onBackPressed();
        //finish();

    }
}
