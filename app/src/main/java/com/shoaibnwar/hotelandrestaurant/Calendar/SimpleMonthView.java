...
class SimpleMonthView extends View
{
    ...
    protected static int RECTANGLE_SZIE;
    protected static int CIRCULE_SIZE;
    ...
    public SimpleMonthView(Context context, TypedArray typedArray)
    {
        ...
        mCurrentDayTextColor = typedArray.getColor(R.styleable.DayPickerView_colorCurrentDay, resources.getColor(R.color.white));
        mMonthTextColor = typedArray.getColor(R.styleable.DayPickerView_colorMonthName, resources.getColor(R.color.normal_day));
        mDayTextColor = typedArray.getColor(R.styleable.DayPickerView_colorDayName, resources.getColor(R.color.white));
        ...
        RECTANGLE_SZIE = typedArray.getDimensionPixelSize(R.styleable.DayPickerView_selectedDayRadius, resources.getDimensionPixelOffset(R.dimen.rect_size));
        CIRCULE_SIZE = typedArray.getDimensionPixelSize(R.styleable.DayPickerView_selectedDayRadius, resources.getDimensionPixelOffset(R.dimen.circle_size));
        ...
        textAbc  = typedArray.getDimensionPixelSize(R.styleable.DayPickerView_selectedDayRadius, resources.getDimensionPixelOffset(R.dimen.rect_size));
    ...
    private void onDayClick(SimpleMonthAdapter.CalendarDay calendarDay) {
        ...
        if (mHasToday && (mToday>calendarDay.day)){
        ...
        /*if (mOnDayClickListener != null && (isPrevDayEnabled || !((calendarDay.month == today.month) && (calendarDay.year == today.year) && calendarDay.day < today.monthDay))) {
            ...
            for (int i = 0; i< CustomCalendar.bookedDates.size(); i++) {
                int mDay = CustomCalendar.bookedDates.get(i).get("day");
                int mM = CustomCalendar.bookedDates.get(i).get("month");
                int mMM = mM-1;
                int mY = CustomCalendar.bookedDates.get(i).get("year");

                if ((calendarDay.day == mDay) && (mMonth == mMM) && (mYear == mY)){
                    isExist = true;
        ...
        while (day <= mNumCells) {
            ...
            if ((mMonth == mSelectedBeginMonth && mSelectedBeginDay == day && mSelectedBeginYear == mYear) || (mMonth == mSelectedLastMonth && mSelectedLastDay == day && mSelectedLastYear == mYear)) {
                if (mDrawRect)
                {
                    ...
                    //canvas.drawRoundRect(rectF, 70.0f, 70.0f, mSelectedCirclePaint);

                    Paint myPaint = new Paint();
                    myPaint.setColor(getResources().getColor(R.color.redish));
                    myPaint.setStrokeWidth(10);

                    //   canvas.drawRect(x-130, y-70, x+130, y+30, myPaint);
                    //66
                    canvas.drawCircle(x, y - RECTANGLE_SZIE /3, RECTANGLE_SZIE, myPaint);
                }
                else
           ...
           if (CustomCalendar.bookedDates.size()>0){
            ...
            for (int i = 0; i<CustomCalendar.bookedDates.size(); i++) {
                ...
                if ((day == mDay) && (mMonth == mMM) && (mYear == mY)){

                    canvas.drawLine(x-30, y-10, x+30, y-20, mSelectedCirclePaint);
                }

            }
           }

            // canvas.drawLine(x-40, y-10, x+40, y-20, mSelectedCirclePaint);
                            ...
                            ((mSelectedBeginMonth > mSelectedLastMonth && mMonth == mSelectedBeginMonth && day < mSelectedBeginDay) || (mSelectedBeginMonth > mSelectedLastMonth && mMonth == mSelectedLastMonth && day > mSelectedLastDay))))
            {
                ...
                if (isAvaila) {
                    ...
                    //canvas.drawCircle(x, y - MINI_DAY_NUMBER_TEXT_SIZE / 3, textAbc, mSelectedCirclePaint);


                    Paint myPaint = new Paint();
                    myPaint.setColor(getResources().getColor(R.color.redish));
                    myPaint.setStrokeWidth(8);
                    double te = textAbc*2.92;
                    int ss = (int)te;
                    canvas.drawRect(x - ss , y - (RECTANGLE_SZIE+18)  , x + ss , y + (RECTANGLE_SZIE-25)  , myPaint);
                   // canvas.drawCircle(x, y - MINI_DAY_NUMBER_TEXT_SIZE / 3, DAY_SELECTED_CIRCLE_SIZE, myPaint);
                    //canvas.drawCircle(x, y - MINI_DAY_NUMBER_TEXT_SIZE / 3, DAY_SELECTED_CIRCLE_SIZE, myPaint);

                }
                else {
                    Log.e("TAg", "the range is not available");
    ...
    protected void initView() {
        ...
        mMonthTitlePaint.setColor(mCurrentDayTextColor);
        ...
        mSelectedCirclePaint.setColor(getResources().getColor(R.color.white));
        mSelectedCirclePaint.setTextAlign(Paint.Align.CENTER);
        mSelectedCirclePaint.setStyle(Paint.Style.FILL);
        mSelectedCirclePaint.setAlpha(SELECTED_CIRCLE_ALPHA);
        mSelectedCirclePaint.setStrokeWidth(10);

        mMonthDayLabelPaint = new Paint();
        mMonthDayLabelPaint.setAntiAlias(true);
        mMonthDayLabelPaint.setTextSize(MONTH_DAY_LABEL_TEXT_SIZE);
        mMonthDayLabelPaint.setColor(mCurrentDayTextColor);
        mMonthDayLabelPaint.setTypeface(Typeface.create(mDayOfWeekTypeface, Typeface.BOLD));
